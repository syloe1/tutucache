...
tutucache 是一个分布式内存缓存库，适用于需要**高并发读取**、**低延迟响应**的场景。它支持多节点部署，通过一致性哈希在节点间分配 key，使用 HTTP 进行节点间通信，并内置了 singleflight 机制防止缓存击穿。

## 特性

- **LRU 淘汰策略**：本地缓存使用 LRU 算法，支持内存上限和淘汰回调
- **一致性哈希**：虚拟节点 + 一致性哈希环，节点增减时最小化缓存迁移
- **Singleflight 合并**：相同 key 的并发请求合并为一次调用，防止击穿
- **Protobuf 通信**：节点间使用 Protocol Buffers 序列化，高效紧凑
- **只读字节视图**：`ByteView` 确保缓存值不会被外部意外修改
- **接口解耦**：通过 `PeerPicker` / `PeerGetter` 接口隔离节点发现和网络通信
...
├── byteview.go         # 只读字节视图
├── cache.go            # 并发安全 LRU 缓存封装
├── peers.go            # 节点选择/通信接口定义
├── http.go             # HTTP 节点池（服务端 + 客户端）
├── geecache.go         # 核心：Group 缓存命名空间
├── geecache_test.go    # 核心测试
├── cmd/
│   └── main.go         # 可运行示例
├── run.sh              # Ubuntu 一键启动脚本
├── run.py              # Windows 一键启动脚本
├── go.mod
└── go.sum
```
